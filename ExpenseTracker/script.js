...
const expenseAmontEl = document.getElementById('expense-amount');
const transactionListEl = document.getElementById('transaction_list');
const transactionFormEl = document.getElementById('transaction-form');
const descriptionEl = document.getElementById('descrip');
const amountEl = document.getElementById('amount');

let transactions = JSON.parse(localStorage.getItem("transact")) || [];
...
function addTransaction (e){
    ...
    localStorage.setItem("transact",JSON.stringify(transactions));
...
function updateSummary(){
    ...
    expenseAmontEl.textContent = formatCurrency(expense); 
}

function formatCurrency(number){
    return new Intl.NumberFormat("en-US", {
        style: "currency",
        currency:"XAF"}).format(number);
}
